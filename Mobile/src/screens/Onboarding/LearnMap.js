...
//MapSelecting Screen
export default function LearnMap({navigation}) {
  ...
  const onPressLockedMap = ({index})=>{
    uri = 'https://englephant.vercel.app/api/map/lock/'+learnerId+'/'+index;
    axios.get(uri)
    .then(function (res) {
      if (res.action === "Mở map")
        axios.post('https://englephant.vercel.app/api/map/unlock/'+learnerId+'/'+index)
        .then(
